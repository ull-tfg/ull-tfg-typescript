...
export class UllDuration {
    ...
    private validate(value: string): void {
        if (value === null) {
            throw new Error(UllDuration.ERROR_NOT_DEFINED);
        }
        value = simplify(value);
        if (value.length === 0) {
            throw new Error(UllDuration.ERROR_EMPTY);
        }
        if (!UllDuration.REGULAR_EXPRESSION.test(value)) {
            throw new Error(UllDuration.ERROR_WRONG_FORMAT);
        }
    }

    /*private parseDuration(): { years: number; months: number; weeks: number; days: number; hours: number; minutes: number; seconds: number } {
        const years = this.getValue().includes('Y') ? parseInt(this.getValue().match(/(\d+)Y/)[1]) : 0;
        const months = this.getValue().includes('M') && !this.getValue().includes('T') ? parseInt(this.getValue().match(/(\d+)M/)[1]) : 0;
        const weeks = this.getValue().includes('W') ? parseInt(this.getValue().match(/(\d+)W/)[1]) : 0;
        const days = this.getValue().includes('D') ? parseInt(this.getValue().match(/(\d+)D/)[1]) : 0;
        const hours = this.getValue().includes('H') ? parseInt(this.getValue().match(/(\d+)H/)[1]) : 0;
        const minutes = this.getValue().includes('M') && this.getValue().includes('T') ? parseInt(this.getValue().match(/(\d+)M/)[1]) : 0;
        const seconds = this.getValue().includes('S') ? parseInt(this.getValue().match(/(\d+)S/)[1]) : 0;
    ...
    public equals(other: any): boolean {
        ...
        if (!(other instanceof UllDuration)) {
            return false;
        }
        const otherDuration: UllDuration = other;
    ...
    public static fromMinutes(value: number): UllDuration {
        ...
        return UllDuration.fromTimeAndUnit(
    ...
    public static fromTimeAndUnit(
        ...
        return new UllDuration(durationString);
    }

    public static random(): UllDuration {
